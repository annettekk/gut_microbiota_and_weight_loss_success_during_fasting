...
```{r}
#Prepare metadata for regression dataframe: create meta dataframe 'bmi_wide'
...
#Keep only 509 OTUs present in Ducarmon data after prevalence filtration:

abs_counts_filtered_g <- otu_T0[ , colnames(otu_T0) %in% colnames(arcsin_sqrt) ]


#Normalise and transform OTU counts
rel_abund_g <- (abs_counts_filtered_g / rowSums(abs_counts_filtered_g))
...
```{r}
#Merge meta dataframe (bmi_wide) and counts dataframe (arcsin_sqrt_g) into regression_data_g dataframe
...
# Define predictor column names - 9 OTUs that were found most important contributors to BMI Change Percent by stepwise regression performed on Ducarmon data
...
pvals <- summary(top10_model_g)$coefficients[, "Pr(>|t|)"]
