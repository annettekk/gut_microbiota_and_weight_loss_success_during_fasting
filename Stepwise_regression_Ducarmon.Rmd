---
title: "Stepwise_regression_Ducarmon"
...
#This we will need later for Shannon alpha-diversity and Bray dissimilarity calculation
...
# Identify the OTUs (columns) that are present in 18 or more participants
...
```{r}
#Normalise counts i.e. convert to relative abundances to account for different number of reads among sample
rel_abund <- (abs_counts_filtered / rowSums(abs_counts_filtered))

#Transform to reduce skewness and heteroskedastisity of counts distribution
arcsin_sqrt <- asin(sqrt(rel_abund))
```

```{r}
#This is an optional step to check normality of counts distribution for each OTU
#Actually, normality of the predictor variable is not required for a linear model. The assumption of normality is for residuals (errors), not the predictors.
...
```{r}
# How does BMI_Change_Percent histogram look?
ggplot(meta_wide, aes(x = BMI_Change_Percent)) +
  geom_histogram(bins = 15, fill = "darkgreen", color = "black", alpha = 0.7) +
  labs(
    title = "Distribution of BMI_Change_Percent",
    x = "BMI_Change_Percent",
    y = "Frequency"
  ) +
  theme_bw()
```
...
```{r}
#This optional chunk shows that absolute BMI_Change correlated with initial BMI. To avoid this confounding factor, BMI_Change_Percent was chosen as Y in linear model

library(ggplot2)

ggplot(meta_wide, aes(x = BMI_Before, y = BMI_Change)) +
  geom_point(color = "steelblue", size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "darkred", linetype = "dashed") +
  labs(
    title = "Relationship Between Absolute BMI Change and Baseline BMI",
    x = "BMI Before",
    y = "BMI Change (After - Before)"
  ) +
  theme_minimal()
...
ggplot(meta_wide, aes(x = BMI_Before, y = BMI_Change_Percent)) +
  geom_point(color = "steelblue", size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE, color = "darkred", linetype = "dashed") +
  labs(
    title = "Relationship Between BMI Change Percent and Baseline BMI",
    x = "BMI Before",
    y = "BMI Change Percent to Baseline BMI"
  ) +
  theme_minimal()
```
```{r}
model_BMI_before <- lm(BMI_Change_Percent ~ BMR, data = regression_data)
summary(model_BMI_before)
```
...
ggplot(meta_wide, aes(x = Fasting_Days, y = BMI_Change_Percent)) +
  ...
  labs(
    title = "BMI Change (%) vs. Fasting Duration, Ducarmon dataset",
    x = "Fasting Days",
    y = "BMI Change (%)"
  ) +
  theme_minimal()
...
ggplot(meta_wide, aes(x = BMR, y = BMI_Change_Percent)) +
  ...
  labs(
    title = "BMI Change (%) vs. Basal Metabolic Rate (BMR), Ducarmon",
    x = "BMR (kcal/day)",
    y = "BMI Change (%)"
  ) +
  theme_minimal()
...
shannon_diversity <- diversity(rel_abund_unfiltered, index = "shannon")
...
# !!! Later in the project, I realised that stepwise regression is not designed for p ≫ n situations (when number of regressors exceeds the number of observations). step() stops adding terms that cause multicollinearity or leave no degrees of freedom for error. It is better to use penalized regression methods (see LASSO bellow) !!!
...
# Plot residuals vs Fasting_Days 
plot(model_data$Fasting_Days, residuals(top10_model),
     xlab = "Fasting_Days",
     ylab = "Residuals",
     main = "Residuals vs Fasting_Days",
     pch = 19, col = "darkorange")
abline(h = 0, col = "red", lty = 2)
...
x <- as.matrix(regression_data[ , !(names(regression_data) %in% c("Study_Patient", "Body_Weight_Before", "Height", "Age", "Sex", "Height_m", "BMI_Before", "BMI_After", "BMI_Change", "BMI_Change_Percent", "BMI_Change_Percent_adj", "Bray_Dissimilarity")) ])
  
y <- regression_data$BMI_Change_Percent
...
# Best lambda (minimum error)
cv_lasso$lambda.min

# Lambda within 1 standard error of min (simpler model)
cv_lasso$lambda.1se
