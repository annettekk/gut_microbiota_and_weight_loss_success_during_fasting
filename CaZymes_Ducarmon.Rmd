...
```{r}
## Create df cazy_rpkm_before that contains CaZyme rpkm counts only at the timepoint before fasting
...
```{r}
## meta_wide is a dataframe that contains metadata for Ducarmon dataset. Data is taken from Ducarmon_2024_Berlin_ps.RDS. meta_wide was created in Stepwise_regression_Ducarmon.Rmd
...
quantile(meta_wide$BMI_Change_Percent, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE)
...
# Non-parametric wilcox.test defaults to the unpaired version when applied to two groups (another name for unpaired Wilcoxon test is Mann–Whitney U test)
...
```
```{r}
# How are counts aggregated into enzyme groups distributed?
# Mucin looks skewed.
